...
try { ...
	...
	logger.error(String.format("payment notice register error - purchaseHist:%s", paymentHistory), e);
}

//BasicPayNoticeBo
mailContentParam.put("amount", Double.parseDouble(iosPrice));
var0 = var0.trim();	//iosPrice
```
# 5. Null 전달 지양
* 의도적으로 null 전달인자를 받도록 구현된 메서드가 아니라면 메서드 인자로 null 전달 지양
* 애초에 메서드 인자로 null 을 전달하는 코드는 잘못된 콛,
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0NDY3OTY0NDcsLTcxNDA0Njg1MiwtMT
AzMDU5MzY3MywtMTg1MDc2MzA0MCwxOTgwMjUwMDUxXX0=
-->