...
* 특수사례객체 : 클래스 메서드에서 발생할 수 있는 예외를 던지기보다 메서드 내에 처리 구문을 포함시키는것
* 감싸기 메서드 : 코드 수정이 불가능한 외부 라이브러리의 API 가 null 대신 예외를 던지도록 수정해주기 위해 외부 라이브러리객체를 멤버변수로 가지는 클래스를 만드는것
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyODYyMjYxMzAsMTk4MDI1MDA1MV19
-->