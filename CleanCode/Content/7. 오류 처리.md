...
public void afterPropertiesSet() throws Exception {  
   if (StringUtils.isEmpty(keyVariable)) {  
      throw new IllegalArgumentException("keyVariable is null");  
  }  
  
  encCipher = Cipher.getInstance(ENC_ALGORITHM);  
  encCipher.init(Cipher.ENCRYPT_MODE, getKey());  
  
  decCipher = Cipher.getInstance(ENC_ALGORITHM);  
  decCipher.init(Cipher.DECRYPT_MODE, getKey());  
}
```
### 감싸기 메서드
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1OTI2NjMwMjgsMTM1Mzk3NDg3MiwtNz
E0MDQ2ODUyLC0xMDMwNTkzNjczLC0xODUwNzYzMDQwLDE5ODAy
NTAwNTFdfQ==
-->