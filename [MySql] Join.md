...
* 데이터 처리량이 적고, 빠른 응답성이 필요한 OLTP 용 쿼리
	> 조인 성공시, 바로 조인결과를 사용자에게 보여줌(스트리밍방식)
...
* PGA 영역에 저장하여 동기화 과정 없이 정렬을 수행하므로 경합 발생하지 않아 효율적
> * PGA(Program Global Area) : 프로세스마다 할당되는 독립적인 메모리공간(일종의 address space) 
> * SGA(System Global Area) : 모든 프로세스가 공유하는 공유메모리공간 
...
5. 정렬된 선행 후행 테이블을 NL 조인하여 추출버퍼에 삽입

### 성능 포인트
* 각 테이블 scan 속도
	* 테이블 scan에 사용가능한 인덱스가 없을경우 성능 저하 발생
* 각 테이블 select 된 row 크기
	* 테이블에서 select 된 row 전체 크기가 소트버퍼 크기보다 클 경우, 임시 파일(디스크)을 만들어 분할 정렬수행하므로 성능 저하 발생 
...
### 성능 포인트
* 각 테이블 select 된 row 크기
	* 해쉬테이블의 크기가 PGA 영역에 설정한 메모리 사이즈(hash_area_size)보다 커질경우 디스크의 임시영역에 저장하여 성능 저하 발생
* 조인키 칼럼 데이터 중복도
