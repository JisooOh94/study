...
### 적용 방법
* 동적으로 확장하고자 하는 어드바이스 메서드를 가지고있는 인터페이스 정의
	* 확장한 메서드 사용시, 인트로듀스 대상 객체 타입인 프록시 객체를 어드바이스 인터페이스 타입으로 형변환하여 호출
...
* ProxyFactory.addAdvisor 로 Minin 클래스 인스턴스 추가하여 객체에 인트로덕션 적용
...
```
4. ProxyFactory.addAdvisor 로 Minin 클래스 인스턴스 추가하여 객체에 인트로덕션 적용
```java
public class Foo {
	Object val;
...
public static void main() {
	...
	ProxyFactory pf = new ProxyFactory();
	pf.setTarget(foo);
	pf.setAdvisor(new DefaultIntroductionAdvisor(new IsModifiedMixIn()));
