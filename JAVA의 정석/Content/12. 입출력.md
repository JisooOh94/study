...
* 직렬화
   * 이러한 필드 데이터들을 연속적인 데이터로 이어붙이는것
   * FileIntputStream객체를 주입받은 ObjectInputStream 사용
   ```
   FileInputStream fis = new FileInputStream("file"
   ```
* 역직렬화 : 스트림으로 전달된 연속적인 데이터들을 객체의 필드에 할당하는것
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU5ODY4MDY3NSwtMjA5Mzk0MTkxMywtMT
