...
### Servlet Non-Blocking I/O 의 한계
* 위와같은 이유로 Servlet 3.1 에서 Non-Blocking I/O를 추가했지만, 기존 Servlet API에 동기식(Filter, Servlet), Blocking(getParameter, getPart) 등 레거시가 많이 남아있어 생산성이 떨어짐 
* 따라서 Spring WebFlux는 Spring MVC와 달리 Servlet과는 전혀 관계없이 만들어졌으며, 그렇기 때문에 더이상 WAS 또한 필요하지 않음(Tomcat 대신 netty 사용)
