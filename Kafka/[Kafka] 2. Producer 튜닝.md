...
> 여기에서 말하는 네트워크 비용엔 다음과 같은 것들이 있다.
> 1. TCP Connection 수립 비용
> 2. 중복되는 Http 헤더 전송 비용
> 3. RTT 시간
...
* compression.type
  ...
  * 압축 방식 종류
  ...
  | 압축 방식 | 장점 | 단점 | Use case |
  |:--:|:--:|:--:|:--:|
  | snappy, lz4 | 빠른 압축 및 해제 속도, 적은 cpu 부하 | 낮은 압축률 | 메시지의 빠른 처리가 필요한 경우 |
  | gzip | 높은 압축률 | 느린 압축 및 해제 속도, 많은 cpu 부하 | 저장공간 및 대역폭 절약이 필요한 경우 |
  | zstd | Kafka 2.1.0 부터 지원하는 가장 최근에 나온 압축방식. <br> gzip 보다 압축률이 높으면서 압축 및 해제 속도도 빠름 | 압축률을 높게 설정할 경우, 많은 CPU 리소스 사용 | Kafka 버전이 2.1.0 이상인 경우 |
